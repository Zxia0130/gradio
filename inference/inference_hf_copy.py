...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        if args.interactive:
            ...
            folder_path = r"/media/a6000/D/Workspace/ZX-GPT/txt_folder"  # 设置文件夹路径
            file_paths = glob.glob(os.path.join(folder_path, "*.txt"))  # 获取所有txt文件的路径
            ...
            def process_file(file_path, batch_size):
                ...
                output_file_name = base_name.replace('.txt', '_processed.txt')  # 生成新的文件名
                output_file_path = os.path.join(processed_folder_path, output_file_name)  # 生成新的文件路径
                input_text_list = []
                
                with open(file_path, 'r', encoding='utf-8') as input_file:
                    text = input_file.read()
                    txt_num = len(text) // batch_size
                    # 分批处理文本
                    for i in tqdm(range(txt_num)):
                        start_index = i * batch_size
                        end_index = start_index + batch_size
                        batch_text = text[start_index:end_index]
                        input_text = batch_text + "以上这段话是使用OCR识别后的文本，但是内容可能不太通顺，请帮我重新整理一下，你只需输出整理后的内容即可。"
                        input_text_list.append(input_text)
                return input_text_list, output_file_path

            # 针对每个文件单独处理
            for file_path in file_paths:
                ask_list = []
                ask_list, output_file_path = process_file(file_path, batch_size=32)
            
                while True:
                    for input_text in tqdm(ask_list):
